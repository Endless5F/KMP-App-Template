package com.jetbrains.kmpapp.compose.adaptive
...
fun NavigationSuiteScaffoldLayout(
    ...
    SubcomposeLayout { constraints ->
        ...
        val navigationPlaceables = subcompose(NavigationSuiteLayoutIdTag) {
            navigationSuite()
        ...
        val navigationSuiteWidth = navigationPlaceables.fastMaxBy { it.width }?.width ?: 0
        val navigationSuiteHeight = navigationPlaceables.fastMaxBy { it.height }?.height ?: 0

        // Find the content composable through it's layoutId tag
        val contentPlaceables = subcompose(ContentLayoutIdTag) {
            val innerPadding = if (isNavigationBar) {
                PaddingValues(bottom = navigationSuiteHeight.toDp())
            } else {
                PaddingValues(start = navigationSuiteWidth.toDp())
            }
            content(innerPadding)
        }.fastMap { it.measure(looseConstraints) }
        ...
        layout(layoutWidth, layoutHeight) {
            ...
            if (isNavigationBar) {
                ...
                navigationPlaceables.fastForEach {
                    it.placeRelative(
                        0,
                        layoutHeight - (navigationSuiteHeight)
                    )
                }
            } else {
                navigationPlaceables.fastForEach { it.placeRelative(0, 0) }
            }
        }
    }
}
